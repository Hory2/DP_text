...
Jak bylo zmíněno výše, vstupy do regulátoru jsou statorový proud v $dq$ souřadnicích $i_{s_{dq13}}$, otáčky motoru $\omega_m$ a generovaný moment $T$. Tyto tři veličiny společně poskytují regulátoru veškeré informace potřebné pro uzavření zpětné vazby jak v proudové, tak v rychlostní smyčce. Na obrázku \ref{fig:regulator blokac} je blokové schéma navrhovaného regulátoru.
...
Při běžném provozu lze výraz kompenzovat i I složkou regulátoru, nicméně při rychlých změnách dosahuje regulátor s kompenzací lepších výsledků. Kompenzace je tedy volitelné vylepšení dynamiky, nikoli podmínka stability.
...
Nulu regulátoru umístím tak, aby kompenzovala pól soustavy, tedy $T_i = L_s / R_s$. Tato metoda kompenzace pólu nulou vede v uzavřené smyčce na systém prvního řádu s jedinou časovou konstantou určenou zvolenou frekvencí řezu. Zesílení $K_p$ jsem volil tak, aby omega řezu odpovídala ${1}/{\tau}$. Výsledný přenos regulátoru převedu do diskrétní podoby s periodou vzorkování $T_s$. Získané hodnoty jsem implementoval do regulátoru v Simulinku podle rovnice \ref{eq:PI diskretni} a manuálně doladil.
...
Pro reálný motor kompenzace nebude nikdy dokonalá, neboť přesnost kompenzace závisí na přesnosti znalosti parametrů modelu, zejména indukčností a odporu vinutí. Proto jsem regulátor i včetně všech nadřazených ladil na systému bez kompenzace a následně přidal kompenzaci, která zlepšila odezvu motoru zejména při rychlých změnách požadovaného proudu. 
...
\label{kap:MTPA}
Cílem tohoto bloku je pro požadovaný moment $T^*$, určený nadřazeným regulátorem rychlosti, vygenerovat požadovaný vektor proudů $i_{dq_{13}}^*$. Volba proudového vektoru má tedy vliv na výsledný moment i na tepelné ztráty v motoru, a právě tento kompromis řeší algoritmus MTPA popsaný dále.
...
Moment je tedy přímo možné ovlivnit proudem první harmonické $i_{dq_1}$. Jelikož $q1$ složka indukčnosti bývá větší než $L_{d1}$, je výraz $\left(L_{\mathrm{d1}}-L_{\mathrm{q1}} \right) $ záporný\cite{petros}. Pro motor s hladkým rotorem ($L_{d1}=L_{q1}$) by reluktanční člen vymizel a moment by závisel pouze na $i_{q1}$.
...
Moment jde ještě zvýšit aplikováním záporného proudu $i_{d1}$. Záporná hodnota $i_{d1}$ způsobuje oslabení výsledného magnetického toku v ose $d$, čímž se uvolňuje prostor pro větší $i_{q1}$ při zachování omezení na celkový proud. Pro požadovaný moment potom existuje vztah mezi $i_{d1}$ a $i_{q1}$, kterým jde daného momentu dosáhnout.


Pro zvolení dvojice proudů lze použít například MTPA (maximum torque per ampere) tedy maximální moment na ampér. Algoritmus spočívá v tom že pro daný moment vybere takovouto dvojici proudů aby byl maximalizován moment generovaný jedním ampérem dodaným do motoru, jinak řečeno minimalizuje odebíraný proud pro daný požadovaný moment.\cite{petros}
...
Na rozdíl od předchozího modelu je generovaný moment nyní ovlivněn také 3.harmonickou složkou proudu. Pokud je proud $i_{q\,3}$ nenulová, motor schopný generovat oproti předchozímu přístupu větší moment. Velikost tohoto přídavného momentu je přímo úměrná konstantě $k$ a proudu $i_{q3}$, jak plyne z rovnice \ref{eq:moment 3rd final}. 



\subsection{Určení referenčního proudu}
K určení vektoru proudu pro požadovaný moment slouží lookup tabulka, která pro požadovaný moment $T^*$ přiřadí odpovídající vektor proudu $i_{dq_{13}}^*$. Použití předpočítané tabulky místo řešení optimalizace za běhu výrazně snižuje výpočetní náročnost a umožňuje deterministické časování řídicího algoritmu.

Hodnoty jednotlivých proudů musí generovat odpovídající moment daný  rovnicí \ref{eq:moment 3rd final}. Cílem je zároveň minimalizovat tepelné ztráty v motoru dané jako $P_{loss} = I^2 \cdot R$, kde $I$ odpovídá celkové velikosti proudu dodaného do motoru. Odpor vinutí $R$ je konstanta a polohu minima neovlivňuje, cílem je tedy minimalizovat druhou mocninu celkového proudu, což odpovídá minimalizaci tepelného výkonu ztrát. Dostávám nákladovou funkci $J$ (rovnice \ref{eq:optimalni proud}), kterou minimalizuji.
...
Zároveň je nutné dodržet omezující podmínky plynoucí z konstrukce střídače, aby nedošlo k přetížení tranzistorů nebo zdroje napájení. Maximální velikost odebíraného proudu je limitována parametry DC zdroje hodnotou $I_{DC\text{max}}$ (podmínka \ref{eq:Idc max}).
...
Tabulka~\ref{tab:porovnani moment} shrnuje dosažitelné maximální momenty
pro tři srovnávané strategie. Nárůst maximálního momentu o přibližně 21\,\% oproti MTPA bez injekce a přibližně o 90\,\% oproti jednoduššímu přístupu. Takto výrazný nárůst momentu ukazuje, že zanedbání prostoru $dq_3$ vede k výraznému nevyužití potenciálu pohonu.

\begin{table}[h]
\centering
\begin{tabular}{lccc}
\toprule
\textbf{Strategie} & $T_{\max}$ [Nm]  & Využití $dq_3$  \\
\midrule
Pouze $i_{q1}$                  &  9{,}8   & ne \\
MTPA bez injekce 3.~harmonické  & 14{,}76 & ne \\
MTPA s injekcí 3.~harmonické    & 17{,}88  & ano \\
\bottomrule
\end{tabular}
\caption{Porovnání strategií generování momentu pro použitý motor}
\label{tab:porovnani moment}
\end{table}
...
Tato MTPA tabulka zajišťuje optimální využití proudu, ale je platná pouze pokud otáčky motoru nepřesahují definovanou mez $\omega_{base}$. Pro vyšší otáčky dochází k růstu zpětného elektromotorického napětí, které může překračovat napětí zdroje. Zpětné EMF roste přibližně lineárně s otáčkami, takže při překročení $\omega_{base}$ lze napěťovou podmínku porušit i při proudu menším než je maximální dovolená hodnota. Výstup proudového regulátoru je saturován na maximu, motor systém není schopen dosáhnout požadovaného proudu, tedy ani momentu. Řešením je oslabování magnetického pole aktivním potlačením magnetického toku $\psi_s$ v ose d1 a d3. Tím dojde ke snížení zpětně indukovaného napětí v ose q1, q3. Díky tomu je pohon schopen generovat proud a tím pádem i moment i při vyšších otáčkách.
