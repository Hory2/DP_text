...
Podle popsaných principů jsem vytvořil Simulink kód a knihovnu jednotlivých bloků, schéma implementovaného řízení je na obrázku \ref{fig:impl_blokove_schema}. Hierarchická organizace do knihovny samostatných bloků mi umožňuje opakované použití a nezávislé testování každého funkčního celku.

\input{obrazky/impl_blokove_schema}

Pro práci jsem použil Matlab Simulink R2025b. Parametry simulace jsou inicializovány ve skriptu \texttt{ParamInit}. Ten využívá některých konstant předpočítaných v jiných skriptech jako  data MTPA tabulky, tabulka pro určení sektoru svm, pole vektorů pro jednotlivé sektory a podobně. Inicializační skript zajišťuje konzistenci parametrů napříč všemi bloky a eliminuje riziko nekonzistentního nastavení při ladění.
...
Protože simscape model Matlabu nesimuluje vliv 3. harmonické byl jsem nucen pro další postup přejít na vlastní model, ten jsem vytvořil využitím popisu motoru v $dq_{1\,3}$.
...
Pro otestované bloky jsem jednotlivě vygeneroval pomocí Matlab HDL coderu (\cite{matlab}) IP bloky, a ty nahrál na ZedBoard a jednotlivě ověřil jejich funkčnost. Na závěr jsem vytvořil jeden IP blok pokrývající celé  řízení. Ověřil že se celé řízení vydá na desku a otestoval jeho funkčnost.
...
Jednotlivé prvky této matice jsou závislé na úhlu $\theta$, musí být proto spočítána pro každý časový okamžik znovu. To by znamenalo 20 přístupů do lookup tabulek. Aby byl omezen počet výpočtů $\sin$ a $\cos$, upravil jsem  matici $K_r$  do tvaru~\ref{mat:Kr hdl}, kde $k_1$ až $k_{5}$ jsou konstanty a ve výrazu se vyskytují pouze hodnoty $\sin(\theta)$, $\cos(\theta)$, $\sin(3\theta)$ a $\cos(3\theta)$. Tato úprava snižuje počet nutných výpočtů $\sin$ a $\cos$ z 20 na 4, neboť každý prvek původní matice obsahuje jiný úhel, zatímco upravený tvar závisí výhradně na těchto čtyřech hodnotách. Tyto hodnoty jsou získány z lookup tabulky předřazené před násobením. Výsledné simulinkové schéma je na obrázku~\ref{fig: ad2dq simulink}.
...
Blok je nejnáročnější ze všech transformačních bloků -- 48 násobiček DSP48E1 (21{,}82\,\%) a 25\,\% dostupných Slice pokrývají výpočet čtyř goniometrických hodnot a jejich násobení s prvky matice $4\times5$.
...
Počet násobiček (48 DSP48E1) je shodný s blokem \texttt{abc2dq}, přestože výstupní rozměr matice je menší -- přesnost signálu napětí (27\,bitů) si vyžaduje stejně široké násobení. Využití LUT a Slice je díky menší matici přibližně čtvrtinové.

\subsubsection{Ověření transformace}
Pro ověření transformace jsem v simulaci roztočil motor kladným směrem působením vnějšího momentu. Vstupy motoru jsem nechal nepřipojené a měřil jsem indukované napětí, to jsem transformoval do $dq$. Správnost převodu se projevila tím, že hodnoty napětí v $dq$ byly konstantní. V tomto zapojení jsem také zkontroloval správný offset magnetického pole statoru vůči snímači otáček, měnil jsem hodnotu offsetu dokud jsem nedosáhl nulové $q$ indukovaného napětí. Při tom jsem zjistil, že simscape model, který jsem chtěl původně použít, negeneruje napětí ve 3.\,harmonické, čímž se potvrdila nutnost použití vlastního modelu.
...
\textbf{Prostředek} & \textbf{Použito} & \textbf{Dostupné} & \textbf{[\%]} \\
\midrule
Slice LUT (celkem)       & 5\,160 & 53\,200  & 9{,}70 \\
\quad z toho: logika     & 4\,971 & 53\,200  & 9{,}34 \\
\quad z toho: dist. RAM  &   128  & 17\,400  & 0{,}74 \\
\quad z toho: shift reg. &    61  & 17\,400  & 0{,}35 \\
Slice Registers (FF)     & 3\,105 & 106\,400 & 2{,}92 \\
Block RAM tile           &     0  &    140   & 0{,}00 \\
DSP48E1 (násobičky)      &    64  &    220   & 29{,}09 \\
Slice                    & 1\,834 & 13\,300  & 13{,}79 \\
\bottomrule
\end{tabular}
\caption{Využití HW prostředků FPGA pro svm výpočet časů  (\texttt{hdl\_svm}).}
\label{tab:hw_hdl_svm}
\end{table}

Modulátor je nejnáročnější na DSP48E1 mezi samostatnými bloky (64 kusů, 29{,}09\,\%), dominuje mu výpočet časů aktivace vektorů násobením s předpočítanými maticemi $A_{n-inv}$. Žádné BRAM nejsou potřeba, neboť look-up tabulky sektoru a předpočítaných matic jsou uloženy v distribuované RAM (LUT RAM).
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\linewidth]{obrazky/Trazisotry.png}
...
\bottomrule
\end{tabular}
\caption{Využití HW prostředků FPGA pro vyvedení výstupu modulátoru (\texttt{HW\_Inteface\_Physical})}
\label{tab:hw_hdl_hwIntrface}
\end{table}

Nejlehčí blok celého řízení -- neobsahuje žádné násobičky (DSP48E1 = 0), protože výpočet spočívá výhradně v porovnání spínacího času s hodnotou čítače. Nároky na LUT a Slice jsou tak nejnižší ze všech bloků.
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.9\linewidth]{obrazky/osciloskop_deadtime.png}
    \caption{Zpoždění mezi nástupem signálu pro horní a dolní tranzistor stejné větve střídače}
    \label{fig: dead time}
\end{figure}


\section{Proudový regulátor}
Pro každou z os je použit vlastní PI regulátor, k akčnímu zásahu je pak ve verzi s kompenzací křížové vazby přičten člen kompenzující křížovou vazbu podle rovnice \ref{eq:substituce v}. Kvůli této kompenzaci je pak přidána saturace i nakonec celého bloku. Schéma je na obrázku \ref{fig:simulink ire}.
...
Diskrétní PI regulátor jsem implementoval ve tvaru dopředné Eulerovy aproximace.
Saturaci I složky jsem nastavil symetricky na úroveň odpovídající maximálnímu
napájecímu napětí, aby bylo zajištěno, že integrátor nepřekročí rozsah
reprezentovatelný datovým typem signálu napětí.
...
Proudový regulátor spotřebovává nejvíce násobiček ze všech bloků (102 DSP48E1, 46{,}36\,\%) -- příčinou je kompenzace křížové vazby, která přidává násobení signálů s 27bitovou šířkou. Bez kompenzace klesne počet DSP48E1 přibližně na polovinu (54 kusů), jak je patrné ze souhrnné tabulky~\ref{tab:hw_utilization_summary}.
...
\caption{Využití HW prostředků FPGA pro generování proudové reference (\texttt{tork\_gen})}
...
Jediný blok, který využívá Block RAM (10 tiles, 7{,}14\,\%), BRAM jsou obsazeny dvěma MTPA vyhledávacími tabulkami. Naopak DSP48E1 je minimální (1 kus), protože blok téměř nenasobí. Výpočetní náročnost spočívá v adresování a čtení tabulek.

\pagebreak
\clearpage
\section{Regulátor otáček}
Regulaci otáček provádí PI regulátor, abych si zajistil plnou kontrolu nad datovými typy, implementoval jsem diskrétní verzi (PS) regulátor sám. Včetně antiwindupu. Aby regulátor zvládal rychle reagovat na změnu zatěžovacího momentu, který do systému z pohledu řízení vstupuje jako porucha, potřebuji výraznou I složku regulátoru, ta však způsobuje velký překmit při náběhu regulátoru, proto jsem i antiwindup naladil výrazně agresivněji.

Abych měl plnou kontrolu nad datovými typy v průběhu výpočtu implementoval jsem diskrétní alternativu PI regulátoru (PS) sám. Regulátor má saturaci I složky a antiwindup. Simulinkové zapojení je na obrázku \ref{fig:pi simulink}.

\begin{figure}[h]
    \centering
    \includegraphics[width= \linewidth]{obrazky/pi_reg.png}
    \caption{Použitá implementace PI regulátoru v Simulinku}
    \label{fig:pi simulink}
\end{figure}
...
\bottomrule
\end{tabular}
\caption{Využití HW prostředků FPGA pro regulátor otáček (\texttt{wreg})}
\label{tab:hw_hdlwreg}
\end{table}

Výpočetně nejlehčí řídicí blok, jediný PI regulátor s antiwudupem vystačí s 10 DSP48E1 (4{,}55\,\%) a zabírá nejméně LUT i Slice ze všech bloků řízení.

Popsaná strategie MTPV umožňující provoz nad základními otáčkami, v rámci této práce ale nebyla implementována. Důvodem je,
že testování probíhalo výhradně v oblasti otáček pod $\omega_{base}$, kde je
MTPA dostačující. Výpočet $\omega_{base}$,  je navíc výpočetně náročnější a pro stávající nastavení datových typů by na ni nestačily výpočetní kapacity.
...
\begin{figure}[h]
    \centering
    \includegraphics[width= 0.8\linewidth]{obrazky/zedBoard.jpg}
    \caption{ZedBoard board při testování výstupu}
    \label{fig:zedboard}
\end{figure}
...
\begin{table}[h]
    ...
    \begin{tabular}{|l|c|c|c|l|}
        ...
        moment ($T$)    & 18                  & ano (1)             & 12                                 & \texttt{fixdt(1,15,8)}  \\ \hline
...
\begin{tabular}{p{4.5cm}rrrrrr}
   ...
   \addlinespace
    SVM \texttt{hdl\_svm}
    & 5\,160 & 3\,105 & 128 & 0 & 64 & 1\,834 \\
    \quad využití [\%]
    & 9{,}70 & 2{,}92 & 0{,}74 & 0{,}00 & 29{,}09 & 13{,}79 \\
   \addlinespace
    Regulátor momentu
