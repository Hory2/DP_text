...
\begin{frame}
	...
	\begin{itemize}
		...
		\item Implementovat algoritmus \textbf{vektorvého řízení} v prostření Matlab/ Simulink
			 \begin{itemize}
			\item \textbf{kaskádní regulátor} -- proudová smyčka + otáčková smyčka
...
\begin{frame}
	...
	\end{figure}
			...
			\begin{itemize}
				...
				\item Schopné porvozu  odolnost pri poruše 
			\end{itemize}
...
\begin{frame}
	...
	\begin{columns}
		...
		\begin{column}{0.6\textwidth}
			\begin{figure}
				\centering
				\input{obrazky/abframe1 copy.tex}
			\end{figure}
		\end{column}
	\end{columns}
\end{frame}
...
\end{equation}
{\input{obrazky/ma5mb5 copy.tex}}
	\end{column}
	\end{columns}
\vspace{0.5ex}
	{\small  $\psi$~-- spřažený tok, $\psi_m$~-- tok PM, $L_{ls}$~-- rozptylová indukčnost, $L_{A5}$, $L_B$~-- amplitudy indukčností, $M_{a5}$, $M_{b5}$~-- matice indukčností, $\Psi_{PM}$~-- amplituda toku PM}
...
\begin{frame}
	...
	\begin{columns}[T]
		\begin{column}{0.56\textwidth}
			...
			\end{equation*}
			\vspace{0.5ex}
			\textbf{Statorový tok a indukčnost:}
			\begin{equation*}
				\psi_{s_{dq}} =
				\underbrace{
				\begin{bmatrix}
...
\begin{frame}
	...
	\input{obrazky/Blokac_riz_prez.tex}
	\begin{itemize}
		\item $T_\mathrm{S\_reg} = 50\,\text{ms}$
	\end{itemize}
...
\begin{frame}
	NENi
	\frametitle{Implementace MTPA s injekcí 3.~harmonické}
	\begin{figure}
	...
	\begin{columns}[T]
		\begin{column}{0.56\textwidth}
			\begin{itemize}
				...
				\item Výstup na fyzické piny (PMOD) -- řídicí signály tranzistorů
				\item \textbf{Dead-time} kompenzace (zamezení zkratu větve)
				\item Datové typy s pevnou desetinnou čárkou (\texttt{fixdt})
			\end{itemize}
		\end{column}
		\begin{column}{0.44\textwidth}
			\begin{figure}
				\centering
				\includegraphics[width=0.9\columnwidth]{obrazky/zedBoard}
				\caption{ZedBoard při testování výstupu}
			\end{figure}
		\end{column}
	\end{columns}
\end{frame}
...
%%%%%%%%%%%%%
% Snímek 10: Využití prostředků FPGA
\begin{frame}
	\frametitle{Využití prostředků FPGA -- souhrn}
	\begin{table}
		\centering
		\small
		\begin{tabular}{lrr}
			\toprule
			\textbf{Prostředek} & \textbf{Použito / Dostupné} & \textbf{[\%]} \\
			\midrule
			Slice LUT               & 13\,933 / 53\,200  & 26{,}2 \\
			Flip-Flop (FF)          & 4\,197 / 106\,400  & 3{,}9  \\
			Block RAM tile          & 10 / 140           & 7{,}1  \\
			\textbf{DSP48E1 (násobiče)} & \textbf{195 / 220} & \textbf{88{,}6} \\
			Slice                   & 4\,361 / 13\,300   & 32{,}8 \\
			\bottomrule
		\end{tabular}
		\caption{Celý systém \texttt{hdl\_celek} -- čip XC7Z020}
	\end{table}
	\vspace{0.5ex}
	\begin{alertblock}{Kritický prostředek: DSP48E1}
		\textbf{88\,\%} násobiček obsazeno. Dominuje proudový regulátor s kompenzací křížové vazby (102~ks, 46\,\%).\\
		Bez kompenzace: $\approx$\,54\,ks (25\,\%) -- kompromis přesnost vs.\ nároky.
	\end{alertblock}
	\vspace{0.5ex}
	\begin{itemize}
		\item BRAM využívány výhradně MTPA lookup tabulkou (\texttt{tork\_gen})
		\item Snížení nároků: úprava přesnosti datových typů za cenu horší regulace
	\end{itemize}
\end{frame}
...
\begin{frame}[plain,noframenumbering]
	...
	\begin{figure}
		\centering
		\includegraphics[width=0.4\textwidth]{obrazky/cross_compens.png}
	\end{figure}
\end{frame}
