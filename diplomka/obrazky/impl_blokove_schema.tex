...
\resizebox{ \textwidth}{!}{%
\begin{tikzpicture}[
    block/.style  = {draw, rectangle, minimum height=1.8cm, minimum width=2.8cm, align=center},
...
\node[block, right=of Ri]      (T1)   {$dq_{13}{\to}\alpha\beta_{13}$\\$(K_r^{inv})$};
\node[block, right=of T1]      (SVM)  {SVM\\modulátor};
\node[block, right=of SVM]     (HW)   {HW interface\\dead\,time};
\node[block, right=1.4cm of HW](M)    {Simulace\\motoru\\};

% Zpetnovazebni transformacni blok
\node[block, below=2.0cm of T1] (T2) {$abcde{\to}dq_{13}$\\$(K_r)$};
...
% omega -> Sw (prima zpetna vazba)
\node[anchor=west, font=\small]
    at ($(M.south)+(0,-1.60)$) {$\omega_m$};
\draw[arrow] let \p1=(M.south), \p2=(Sw.south) in
    (\x1,\y1-1.60cm) -- (\x2,\y1-1.60cm) -- (Sw.south);

\end{tikzpicture}%
}
\caption{Blokové schéma implementace řídícího algoritmu}
\label{fig:impl_blokove_schema}
\end{figure}
